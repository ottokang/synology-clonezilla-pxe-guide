...
使用 ocs_repository 來設定映像檔儲存區，參數說明：
...
使用 ocs_live_run 參數來設定自動儲存 / 還原映像檔，參數說明：<https://clonezilla.org/fine-print-live-doc.php?path=./clonezilla-live/doc/98_ocs_related_command_manpages/01-ocs-sr.doc>
...
### Q1. 儲存 Windows 作業系統的映像檔出現錯誤

先進行 chkdsk 掃描磁碟後，再進行儲存

### Q2. 從大硬碟還原到小硬碟出現錯誤

建議不要從大硬碟到小硬碟進行還原，容易出現錯誤，如果一定要還原，建議調整原本來源電腦的磁碟分區大小，小於目標硬碟後再進行還原，調整磁碟分區可以使用 [Gparted](https://gparted.org/)
